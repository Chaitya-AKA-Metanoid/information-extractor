...
def extract_text_from_uploaded_file(uploaded_file):
    """Reads text from the Streamlit uploaded file object."""
    try:
...
def clean_and_segment_text(raw_text):
    """Cleans up line breaks and segments text into sentences."""
    cleaned_text = raw_text.replace('-\n', '') 
    cleaned_text = re.sub(r'\s*\n\s*', ' ', cleaned_text).strip()
    return sent_tokenize(cleaned_text)

def find_sentence(sentences, keywords):
    """Returns the first sentence containing ALL keywords in the list."""
...
def get_entity(text, label):
    """Extracts the first entity of a specific label from text."""
    doc = nlp(text)
...
if uploaded_file is not None:
    with st.spinner("Processing document..."):
        ...
        if raw_text:
            ...
            # 3. Reset Globals & Run Extraction
            FINAL_DATA_ROWS = []
            ROW_COUNTER = 1
